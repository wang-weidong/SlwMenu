	...
	SlwPlugins.SlwMenu.prototype = {
		...
		initEvent : function() {
			...
			this.object.find('>ul>li>a').on('mousemove touchmove', function(e) {
				...
				if (checkFlag) {
					that.unbindEvent();
					if (that.floatMenu.is(":visible")) $(document).unbind('mousedown.slwMenu');
					that.beforeEl = this;
					if ($ul.attr('class') == that.option.className[2]) {
						...
						$sub.show();
						$(document).bind('mousedown.slwMenu', function(e) {
							setTimeout(function() {
								that.hide();
							}, 50);
		...
		hide : function() {
			$(document).unbind('mousedown.slwMenu');
			this.floatMenu.hide();
			this.unbindEvent();
			this.beforeEl = this.floatMenu[0];
		},
		...
		initData : function(dataArray, el, subFlag) {
			...
			for (var i = 0; i < dataArray.length; i++) {
				...
				var $a = $('<a></a>');
				var href = Slw.Utils.isnull(data.path) ? 'javascript:;' : data.path;
				$a.attr('href', href);
